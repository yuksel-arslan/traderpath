...
| 2026-02-16 | Market Pulse HTTP 451 hatası - analiz başlatılamıyordu | `fetch24hTicker()` ve `fetchKlines()` hata durumunda throw ediyordu (diğer tüm fonksiyonlar fallback döndürüyordu). Binance HTTP 451 döndürdüğünde `Promise.all` crash oluyordu. Fix: 1) `fetch24hTicker()` artık fallback MarketData döndürüyor (price: 0, change: 0), 2) `fetchKlines()` artık boş dizi döndürüyor. `calculateTrend()` zaten boş/kısa dizi için `{ direction: 'neutral', strength: 50 }` döndürüyordu | `analysis.engine.ts:2370-2396,2364-2367` |
