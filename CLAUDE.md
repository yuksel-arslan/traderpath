...
| 2026-02-16 | Market Pulse HTTP 451 hatası - analiz başlatılamıyordu | `fetch24hTicker()`, `fetchKlines()`, `fetchOrderBook()`, `fetchRecentTrades()` hata durumunda throw ediyordu (diğer tüm fonksiyonlar fallback döndürüyordu). Binance HTTP 451 döndürdüğünde `Promise.all` crash oluyordu. Fix: 1) `fetch24hTicker()` artık fallback MarketData döndürüyor (price: 0, change: 0), 2) `fetchKlines()` artık boş dizi döndürüyor, 3) `fetchOrderBook()` artık `{ bids: [], asks: [] }` döndürüyor, 4) `fetchRecentTrades()` artık boş dizi döndürüyor. Tüm `fetchWithRetry` çağrıları artık try-catch içinde | `analysis.engine.ts` |
