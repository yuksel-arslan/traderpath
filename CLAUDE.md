...
| 2026-02-02 | Mind map Layer 2'de market kartlarına tıklayınca fullscreen modal açılmıyordu | `SystemFlowChart` komponentine `onLayerClick` callback prop eklendi. Layer 2'deki market kartları (CRYPTO, STOCKS, BONDS, METALS) tıklanabilir hale getirildi - `cursor-pointer`, `hover:scale-105`, hover'da cyan border ve ChevronRight ikonu gösterimi. Karta tıklandığında `onLayerClick(2)` çağrılarak fullscreen Market Flow Analyzer modal'ı açılıyor | `capital-flow/page.tsx` |
