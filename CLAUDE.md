...
| 2026-01-19 | TFT Predictor build timeout (europe-west4) | CPU-only PyTorch kullan (~5GB→~1GB), CUDA deps gereksiz çünkü Railway GPU yok | `services/tft-predictor/requirements.txt`, `Dockerfile`, `railway.toml` |
...
- PyTorch CPU-only olarak değiştirildi (--extra-index-url https://download.pytorch.org/whl/cpu)
- Docker image boyutu ~5GB'dan ~1GB'a düşürüldü
- CUDA dependencies kaldırıldı (Railway GPU desteklemiyor)
- CUDA_VISIBLE_DEVICES="" env var eklendi
- Healthcheck timeout 60s→120s artırıldı
