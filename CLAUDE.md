...
| 2026-02-02 | Mind map'te layer'lara tıklandığında fullscreen modal açılmıyordu | `SystemFlowChart` komponentine `onLayerClick` callback prop eklendi. Tüm layer header'larının onClick handler'ları hem `toggleLayer()` hem de `onLayerClick()` çağıracak şekilde güncellendi. Ana sayfada `setFullscreenLayer` callback olarak geçiriliyor | `capital-flow/page.tsx` |
