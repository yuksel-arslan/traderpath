...
| 2026-02-16 | Signal routes 500 error - 5 endpoint'te try/catch eksikti | `GET /signals`, `GET /signals/:id`, `GET /signals/latest/:market`, `GET /signals/preferences`, `PATCH /signals/preferences` route'larına try/catch eklendi. DB tablosu yoksa (P2021) graceful fallback döndürülüyor (boş dizi/default prefs) | `signal.routes.ts` |
| 2026-02-16 | signals/user_signal_preferences/signal_subscriptions tabloları production DB'de yok - sürekli P2021 hataları (BILGE'de 9+ hata) | **4 dosyada P2021 graceful degradation eklendi**: 1) `signal.service.ts`: Tüm 12 Prisma metodu try/catch + `isTableMissing()` guard ile sarıldı - tablo yoksa boş dizi/0/null/default prefs döndürüyor, `logTableMissingOnce()` ile log spam önlendi, 2) `signal-outcome-tracker.job.ts`: `trackSignalOutcomes()` ve `sendUserNotifications()` P2021 koruması eklendi - tablo yoksa early return, 3) `signal-generator.job.ts`: `checkSignalsTableExists()` ile startup check eklendi - tablo yoksa analiz API çağrıları yapılmadan early return (maliyet tasarrufu), 4) `signal-subscription.service.ts`: `getUserSignalSubscription()` P2021'de `DEFAULT_FREE_SUBSCRIPTION` döndürüyor | `signal.service.ts`, `signal-outcome-tracker.job.ts`, `signal-generator.job.ts`, `signal-subscription.service.ts` |
...
| 2026-02-16 | Analysis routes 11 NaN propagation bug (indicator calculations) | 7 EMA hesabında (Keltner, Force Index, Elder Ray, DEMA, TEMA, TRIX, PPO) `arr[i]`/`closes[i]` undefined olduğunda NaN yayılması. CMO `closes[i]-closes[i-1]`, DPO `closes[i-shift]`, Coppock ROC `closes[i]/closes[i-roc]` division, Volume Profile `vaVolume` init. Tümüne `?? 0` guard eklendi | `analysis.routes.ts` |
| 2026-02-16 | indicators.service.ts 23 runtime bug | `Math.max/min(...arr)` boş dizi crash, 12 division-by-zero (EMA/VWMA/VWAP/ROC/UO/Bollinger/Keltner/HV/ForceIndex/PVT/RVOL/OrderFlow strength hesapları), Slippage/MarketImpact division, spoofing wickRatio, calculateIndicator name null guard | `indicators.service.ts` |
| 2026-02-16 | indicator-interpreter.service.ts 3 runtime bug | Bollinger bandwidth/percentB `upper-lower=0` division, `Math.max/min(...recentPrices)` ve `Math.max/min(...recentMacd)` boş dizi crash | `indicator-interpreter.service.ts` |
| 2026-02-16 | analysis-orchestrator.service.ts 4 runtime bug | MACD `histogram.toFixed()` undefined üzerinde, Bollinger `percentB` null check eksik, ATR `atrPercent` NaN guard, Stochastic `k` null/NaN guard | `analysis-orchestrator.service.ts` |
...
| 2026-02-16 | Email raporu gönderilemiyordu - "An unexpected error occurred" | **3 kök neden**: 1) Fastify default body limit 1MB - büyük PDF/screenshot base64 413 hatası veriyordu → email-screenshot ve email-pdf-report endpoint'lerine `bodyLimit: 20MB` eklendi, 2) Resend API attachment'ında `content_type` eksikti → `application/pdf` eklendi, 3) Frontend generic hata mesajı gösteriyordu → backend'den gelen gerçek hata mesajı gösterilecek şekilde düzeltildi | `report.routes.ts:1524-1641,1657-1785`, `email.service.ts:480-494`, `details/[id]/page.tsx:206,429` |
| 2026-02-16 | PDF rapor dark mode'da oluşuyordu, mum grafiği görünmüyordu | **3 düzeltme**: 1) `handleDownloadPdf`, `handleEmailReport`, `handleAutoEmail` ve `handleAutoPdf` fonksiyonlarında `backgroundColor: '#0f172a'` → `'#ffffff'` ve `classList.add('dark')` → `classList.remove('dark')` olarak değiştirildi, 2) Canvas'lar DOM'dan kaldırılıyordu (chart kayboluyordu) → `replaceCanvasesWithImages()` fonksiyonu eklendi: canvas'ları toDataURL ile statik img'ye çeviriyor, html2canvas bu img'yi yakalıyor, sonra canvas geri yükleniyor, 3) reports/[id]/page.tsx'de de aynı düzeltmeler uygulandı (5 html2canvas çağrısı) | `details/[id]/page.tsx:68-100,162-313`, `reports/[id]/page.tsx:192-466` |
