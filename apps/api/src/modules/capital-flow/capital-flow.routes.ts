...
export async function capitalFlowRoutes(app: FastifyInstance) {
   ...
   *
   * Returns AI-recommended assets based on current L1-L4 Capital Flow data.
   * Builds a session-specific recommendation with alignment scores,
   * risk tags, and warnings for the top-down analysis flow.
   *
  ...
  app.post('/recommend-assets', {
    ...
    try {
      ...
      // Generate a unique session ID for this recommendation
      const capitalFlowId = Date.now().toString(36) + Math.random().toString(36).substring(2, 8);
      const timestamp = new Date();

      // L1 Status: Global Liquidity
      const l1Status = {
        bias: summary.liquidityBias,
        netLiquidityChange30d: summary.globalLiquidity?.netLiquidity?.change30d ?? 0,
        dxyTrend: summary.globalLiquidity?.dxy?.trend ?? 'stable',
        vixLevel: summary.globalLiquidity?.vix?.level ?? 'neutral',
      };

      // L2 Status: Market Flows
      const l2Status = (summary.markets || [])
        .filter((m: MarketFlow) => m && m.market)
        .map((m: MarketFlow) => ({
          market: m.market,
          phase: m.phase || 'mid',
          flow7d: m.flow7d ?? 0,
      ...
      // L3 Status: Sector Drill-Down
      const primaryMarket = summary.recommendation?.primaryMarket || 'crypto';
      const primaryMarketFlow = (summary.markets || []).find((m: MarketFlow) => m.market === primaryMarket);
      const l3Status = {
        primaryMarket,
        sectors: (primaryMarketFlow?.sectors || []).map((s: { name: string; trending: string; flow7d?: number }) => ({
          name: s.name,
          trending: s.trending,
          flow7d: s.flow7d ?? 0,
        })),
      ...
      // L4 Status: Recommendations
      const buyRec = summary.recommendation;
      const sellRec = summary.sellRecommendation || null;

      const l4Status = {
        buyRecommendation: buyRec ? {
          market: buyRec.primaryMarket,
          confidence: buyRec.confidence ?? 0,
          action: buyRec.action,
          suggestedAssets: (buyRec.suggestedAssets || []).map((a: { symbol: string; name: string }) => a.symbol),
        } : null,
        sellRecommendation: sellRec ? {
          market: sellRec.primaryMarket,
          confidence: sellRec.confidence ?? 0,
          action: sellRec.action,
          suggestedAssets: (sellRec.suggestedAssets || []).map((a: { symbol: string; name: string }) => a.symbol),
        } : null,
      };

      // Build recommended assets array from both BUY and SELL recommendations
      const recommendedAssets: Array<{
        symbol: string;
        name: string;
        market: MarketType;
        direction: 'BUY' | 'SELL';
      ...
      if (buyRec && buyRec.suggestedAssets) {
        for (const asset of buyRec.suggestedAssets) {
          recommendedAssets.push({
            symbol: asset.symbol,
            name: asset.name,
            market: asset.market || buyRec.primaryMarket,
            direction: 'BUY',
            confidence: buyRec.confidence ?? 0,
            alignmentScore: buyRec.fiveFactorScore?.totalScore ?? 0,
            riskTag: asset.riskLevel || 'medium',
            reason: asset.reason || buyRec.reason || '',
          });
        }
      }

      if (sellRec && sellRec.suggestedAssets) {
        for (const asset of sellRec.suggestedAssets) {
          recommendedAssets.push({
            symbol: asset.symbol,
            name: asset.name,
            market: asset.market || sellRec.primaryMarket,
            direction: 'SELL',
            confidence: sellRec.confidence ?? 0,
            alignmentScore: sellRec.fiveFactorScore?.totalScore ?? 0,
            riskTag: asset.riskLevel || 'medium',
            reason: asset.reason || sellRec.reason || '',
          });
        }
      }
      ...
      if (summary.liquidityBias === 'risk_off') {
        warnings.push('L1 Risk-Off: Only safe haven assets recommended');
      }

      for (const mf of l2Status) {
        if (mf.phase === 'exit') {
          warnings.push(`Market in EXIT phase: No new entries for ${String(mf.market).toUpperCase()}`);
        } else if (mf.phase === 'late') {
          warnings.push(`Market in LATE phase: Caution advised for ${String(mf.market).toUpperCase()}`);
        }
      }

      if (summary.globalLiquidity?.vix?.level === 'extreme_fear') {
        warnings.push('VIX at extreme fear level: High volatility expected');
      }

      if (summary.globalLiquidity?.yieldCurve?.inverted) {
        warnings.push('Yield curve inverted: Recession risk elevated');
      }

      // Determine if analysis can proceed
      const canProceed = recommendedAssets.length > 0 &&
        (buyRec?.action !== 'avoid' || (sellRec !== null && sellRec.action !== 'avoid'));
